...
class SearchViewModel: ObservableObject {
    ...
    func fetchUsers() async {
        ...
        let query = FirestoreConstants.UserCollection.limit(to: 99)
        ...
        if let last = lastDoc { //TODO: integrate with UserService Cache, single source of truth + this paging logic can be used for cache
          ...
          guard let snapshot = try? await next.getDocuments(source: .default) else { return }
            self.lastDoc = snapshot.documents.last
            self.users.append(contentsOf: snapshot.documents.compactMap({ try? $0.data(as: User.self) }))
        } else {
          guard let snapshot = try? await query.getDocuments(source: .default) else { return }
            self.lastDoc = snapshot.documents.last
